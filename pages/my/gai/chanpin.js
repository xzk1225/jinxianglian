...
Page({
  ...
  this.setData({
    ...
    reques.getdata({ data: 'content/info' }, { access_token: token, id:options.id}).then(res=>{
      that.setData({
        ...
        invest_deadline: res.data.result.invest_deadline - 1,
        way: res.data.result.way - 1,
        age: res.data.result.age - 1 ,
        security_deadline:res.data.result.security_deadline -1,
        loan_type: res.data.result.loan_type - 1 ,
        loan_deadline:res.data.result.loan_deadline - 1 ,
        product_type: res.data.result.product_type -1,
        domain: res.data.result.domain-1,
  ...
  del(){
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          reques.getdata({ data: 'content/del' }, { access_token: wx.getStorageSync('token'), id: this.data.id }).then(res => {
  ...
  formSubmit:function(e){
    ...
    reques.getdata({ data: 'content/update' }, e.detail.value).then(res=>{
      console.log(res.data)
      if(res.data.code==1000){
        reques.nav('we')
      }
